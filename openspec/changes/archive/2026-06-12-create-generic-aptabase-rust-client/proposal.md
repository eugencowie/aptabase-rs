...
- Expose a public client API for configuration, event tracking, asynchronous flushing, and optional periodic flushing.
- Keep runtime-independent blocking client support out of scope; callers use the asynchronous API from a Tokio runtime.
- Replace WebView-specific metadata collection with generic Rust application metadata while preserving the Aptabase event payload contract.
...
- `generic-rust-client`: Configure Aptabase, enqueue typed events, manage sessions, batch and retry ingestion requests, and flush analytics from non-Tauri Rust applications.
...
- The Tauri-specific portions of `src/lib.rs`, all of `src/commands.rs`, `build.rs`, `permissions/`, `webview-src/`, `webview-dist/`, `examples/helloworld/`, and JavaScript package metadata will be removed.
